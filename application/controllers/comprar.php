...
class Comprar extends CI_Controller{
    ...
    function revisao() {
 	 	$usuario = array(
    		...
    		'nome'       => $this->ion_auth->user()->row()->username,
            // 'ddd'        => '21', // só números
            'telefone'   => $this->ion_auth->user()->row()->telefone, // só números
            // 'email'      => $this->ion_auth->user()->row()->email,
            'email'      => "allan@sandbox.pagseguro.com.br",
            'shippingType' => 3, //1=Encomenda normal (PAC), 2=SEDEX, 3=Tipo de frete não especificado.
            'cep'        => $this->ion_auth->user()->row()->cep,      // só números
            'logradouro' => $this->ion_auth->user()->row()->endereco,
            'numero'     => $this->ion_auth->user()->row()->numero,
            'compl'      => $this->ion_auth->user()->row()->complemento,
            'bairro'     => $this->ion_auth->user()->row()->bairro,
            'cidade'     => $this->ion_auth->user()->row()->cidade,
            'uf'         => $this->ion_auth->user()->row()->user_id,
            'pais'       => 'BRA'
            );
    ...
    public function retorno_pagseguro() {
    	...
    	if (count($_POST) > 0) {
            ...
            if ($notificationType && $notificationCode) {
                 ...
                 */          
                if($not['status'] == "Paga") {
                    $this->ingles->setOrderApproved($not['reference']);
                }
                $this->debug($not);
            ...
            if ($retorno == "VERIFICADO") {              
            	...
            	if ($_POST['StatusTransacao'] == 'Aprovado') {
            		...
            		$informacao['status'] = '4';
                    $this->ingles->setOrderApproved($_POST['Referencia']);
    ...
    public function debug($array) {
    	...
    	write_file(APPPATH . "cache/pagseguro_" . date("Y-m-d_h-i") . ".php", json_encode($array));
    }
